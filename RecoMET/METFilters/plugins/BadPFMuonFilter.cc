...
BadPFMuonFilter::filter(edm::StreamID iID, edm::Event& iEvent, const edm::EventSetup& iSetup) const
{
  ...
  for ( unsigned i=0; i < muons->size(); ++i ) { // loop over all muons
    ...
    if ((muon::segmentCompatibility(muon) > 0.3 and bestMuonTrack->ptError()/bestMuonTrack->pt() < 2.0 )) {
      if (debug_) cout <<"Skipping this muon because segment compatiblity > 0.3 or relErr(best track) <2 " << endl;
      continue;
    }
